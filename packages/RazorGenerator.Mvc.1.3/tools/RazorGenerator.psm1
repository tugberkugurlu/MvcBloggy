#NOTE: There are some functions here which are not originally part of the package
#I added them. 2012-01-08 13:03
...
function Set-BuildAction {
    ...
    if($force -or ($buildActionProperty.Value -ne 0)) { 
...
function Enable-RazorGenerator {
    param(
        [parameter(ValueFromPipelineByPropertyName = $true)]
        [string]$ProjectName, [boolean]$isBuildActionNone
    )
    Process {
        ...
        Get-RazorFiles $ProjectName | % { 
            if (Set-CustomTool $_ "RazorGenerator") {
                ...
                if($isBuildActionNone) {
                    Set-BuildAction $_ | Out-Null
                }
